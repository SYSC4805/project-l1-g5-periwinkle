...
const int blacklevl = 810; // threshold for detecting front black line
...
void obstacleTask(void *pv)
{
    ...
    for (;;)
    {
        ...
        if (duration_m > 0)
            distance_cm_m = (duration_m * 0.0343f) / 2.0f;

        if (distance_cm_m > 0 && distance_cm_m < distancethresh)
            ussNearCount_m++;
        else
            ussNearCount_m = 0;
        ...
        if (duration_r > 0)
            distance_cm_r = (duration_r * 0.0343f) / 2.0f;

        if (distance_cm_r > 0 && distance_cm_r < distancethresh)
            ussNearCount_r++;
        else
            ussNearCount_r = 0;
        ...
        if (duration_l > 0)
            distance_cm_l = (duration_l * 0.0343f) / 2.0f;

        if (distance_cm_l > 0 && distance_cm_l < distancethresh)
            ussNearCount_l++;
        else
            ussNearCount_l = 0;
    ...
    for (;;)
    {
        if (xQueueReceive(motorQueue, &cmd, portMAX_DELAY) == pdTRUE)
        {
            ...
            else
            {
                ...
                if (USSDetected_m && USSDetected_r && USSDetected_l)
                {
                    ...
                    else if (USSdistance_l < USSdistance_m)
                    {
                        // Serial.print("Middle & Right USS: Turn Right ");
                        turnright();
                        vTaskDelay(pdMS_TO_TICKS(500));
                    }
                    else
                ...
                else if (obstacleDetected && !USSDetected_m && !USSDetected_r && !USSDetected_l)
                {
                    goforward();
